...
st.caption(
    ...
    if DEMO_MODE else
    "**Live mode** — connected to vLLM inference server."
)
...
with st.sidebar:
    ...
    st.divider()
    adapter_a = st.selectbox("Adapter A", list(ADAPTER_PERSONAS.keys()), index=1, key="adapter_a")
    if mode == "⚔️ A/B Comparison":
        adapter_b = st.selectbox("Adapter B", list(ADAPTER_PERSONAS.keys()), index=0, key="adapter_b")
...
# ── Streaming helper ───────────────────────────────────────────────────────────
def _stream_openrouter(messages: list[dict], adapter_name: str, or_model: str) -> tuple[str, float, int]:
...
def _call_vllm(prompt: str, adapter: str) -> tuple[str, float, int]:
    ...
    r = httpx.post(
        ...
        json={"prompt": prompt, "model": adapter, "max_tokens": max_tokens, "temperature": temperature, "top_p": top_p},
        timeout=60,
    )
    r.raise_for_status()
    d = r.json()
...
if mode == "💬 Single Chat":
    ...
    st.markdown(f"<span class='model-badge'>🎯 {adapter_a}</span><span class='model-badge'>⚡ {or_model_label}</span>", unsafe_allow_html=True)

    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]):
            st.markdown(msg["content"])
            if "latency_ms" in msg:
                st.markdown(f"<span class='latency-tag'>⏱ {msg['latency_ms']:.0f}ms · {msg.get('tokens','?')} tokens</span>", unsafe_allow_html=True)
    ...
    if user_input := st.chat_input("Ask anything…"):
        ...
        with st.chat_message("assistant"):
            try:
                ...
                if DEMO_MODE:
                    text, latency_ms, tokens = _stream_openrouter(st.session_state.messages, adapter_a, or_model_id)
                else:
                ...
                st.markdown(f"<span class='latency-tag'>⏱ {latency_ms:.0f}ms · {tokens} tokens · {tps:.0f} tok/s</span>", unsafe_allow_html=True)
                st.session_state.last_stats = {"latency_ms": latency_ms, "tokens": tokens, "tps": tps}
                st.session_state.messages.append({"role": "assistant", "content": text, "latency_ms": latency_ms, "tokens": tokens})
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_a:
        st.markdown(f"<div class='ab-left'><b>Model A</b> — <span style='color:#a5b4fc'>{adapter_a}</span></div>", unsafe_allow_html=True)
    with col_b:
        st.markdown(f"<div class='ab-right'><b>Model B</b> — <span style='color:#6ee7b7'>{adapter_b}</span></div>", unsafe_allow_html=True)
        ...
        if i < len(msgs_a):
            ...
            with c1:
                with st.chat_message(m["role"]):
                    ...
                    if "latency_ms" in m:
                        st.markdown(f"<span class='latency-tag'>⏱ {m['latency_ms']:.0f}ms</span>", unsafe_allow_html=True)
        if i < len(msgs_b):
            m = msgs_b[i]
            with c2:
                with st.chat_message(m["role"]):
            ...
            with st.chat_message("assistant"):
                try:
                    if not OPENROUTER_KEY:
                        ...
                        text_a, lat_a, tok_a = _stream_openrouter(st.session_state.messages, adapter_a, or_model_id)
                        tps_a = tok_a / (lat_a / 1000) if lat_a > 0 else 0
                        st.markdown(f"<span class='latency-tag'>⏱ {lat_a:.0f}ms · {tok_a} tok · {tps_a:.0f} tok/s</span>", unsafe_allow_html=True)
                        st.session_state.messages.append({"role": "assistant", "content": text_a, "latency_ms": lat_a, "tokens": tok_a})
                except Exception as e:
                    st.error(str(e))
            ...
            with st.chat_message("assistant"):
                try:
                    if not OPENROUTER_KEY:
                        ...
                        text_b, lat_b, tok_b = _stream_openrouter(st.session_state.messages_b, adapter_b, or_model_id)
                        tps_b = tok_b / (lat_b / 1000) if lat_b > 0 else 0
                        st.markdown(f"<span class='latency-tag'>⏱ {lat_b:.0f}ms · {tok_b} tok · {tps_b:.0f} tok/s</span>", unsafe_allow_html=True)
                        st.session_state.messages_b.append({"role": "assistant", "content": text_b, "latency_ms": lat_b, "tokens": tok_b})
                except Exception as e:
                    st.error(str(e))
...
with st.expander("📋 Adapter Stats"):
    STATS = {
        "base":                       {"mmlu": "54.0%", "judge": "5.8/10", "gpu": "38.4 GB", "rank": "—",  "method": "Base"},
        "llama3-qlora-r32":           {"mmlu": "63.0%", "judge": "7.1/10", "gpu": "16.8 GB", "rank": "32", "method": "QLoRA"},
        "mistral-qlora-best":         {"mmlu": "68.0%", "judge": "7.6/10", "gpu": "15.9 GB", "rank": "64", "method": "QLoRA"},
        "llama3-qlora-final-champion":{"mmlu": "71.0%", "judge": "7.9/10", "gpu": "16.1 GB", "rank": "64", "method": "QLoRA"},
    }
    cols = st.columns(4)
