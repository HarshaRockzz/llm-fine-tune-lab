...
st.caption("MMLU, TruthfulQA, LLM-as-Judge scores — baseline vs. fine-tuned champion model.")
...
m1.metric("MMLU Accuracy", f"{ft['mmlu_overall']:.1%}", f"+{(ft['mmlu_overall'] - base['mmlu_overall']) * 100:.1f}pp")
m2.metric("TruthfulQA MC1", f"{ft['truthfulqa_mc1']:.1%}", f"+{(ft['truthfulqa_mc1'] - base['truthfulqa_mc1']) * 100:.1f}pp")
m3.metric("TruthfulQA MC2", f"{ft['truthfulqa_mc2']:.1%}", f"+{(ft['truthfulqa_mc2'] - base['truthfulqa_mc2']) * 100:.1f}pp")
m4.metric("LLM Judge (/ 10)", f"{ft['judge_composite']:.1f}", f"+{ft['judge_composite'] - base['judge_composite']:.1f}")
m5.metric("Judge Pass Rate", f"{ft['judge_pass_rate']:.0%}", f"+{(ft['judge_pass_rate'] - base['judge_pass_rate']) * 100:.0f}pp")
...
fig_mmlu = go.Figure()
fig_mmlu.add_trace(go.Bar(name="Base Model", x=cats, y=base_scores, marker_color="#475569"))
fig_mmlu.add_trace(go.Bar(name="QLoRA Fine-tuned", x=cats, y=ft_scores, marker_color="#6366f1"))
...
fig_ckpt = make_subplots(rows=1, cols=2, subplot_titles=("MMLU Accuracy Progress", "Eval Loss Progress"))

fig_ckpt.add_trace(go.Scatter(
    x=ckpt["step"], y=ckpt["mmlu"],
    name="MMLU", line=dict(color="#6366f1", width=2.5),
    mode="lines+markers", marker=dict(size=5),
), row=1, col=1)
fig_ckpt.add_trace(go.Scatter(
    x=ckpt["step"], y=ckpt["custom"],
    name="Custom Domain", line=dict(color="#10b981", width=2.5),
    mode="lines+markers", marker=dict(size=5),
), row=1, col=1)
fig_ckpt.add_hline(y=0.54, line_dash="dash", line_color="#94a3b8", annotation_text="Baseline 54%", row=1, col=1)
fig_ckpt.add_hline(y=0.71, line_dash="dash", line_color="#10b981", annotation_text="Target 71%", row=1, col=1)

fig_ckpt.add_trace(go.Scatter(
    x=ckpt["step"], y=ckpt["eval_loss"],
    name="Eval Loss", line=dict(color="#f59e0b", width=2.5),
    mode="lines+markers", marker=dict(size=5), showlegend=True,
), row=1, col=2)
...
with col_bar:
    ...
    fig_judge_bar = px.bar(
        df_judge, x="Score", y="Dimension", color="Model", orientation="h", barmode="group",
...
i1.metric("vLLM Throughput", f"{vllm['throughput_tok_per_sec']:,} tok/s", f"{speedup:.1f}× vs HF")
i2.metric("vLLM P50 Latency", f"{vllm['p50_latency_ms']:,} ms", f"-{hf['p50_latency_ms'] - vllm['p50_latency_ms']:,} ms")
i3.metric("vLLM P95 Latency", f"{vllm['p95_latency_ms']:,} ms", f"-{hf['p95_latency_ms'] - vllm['p95_latency_ms']:,} ms")
i4.metric("vLLM GPU Util", f"{vllm['gpu_util_pct']}%", f"+{vllm['gpu_util_pct'] - hf['gpu_util_pct']}%")

infer_df = pd.DataFrame([
    {"Metric": "Throughput (tok/s)", "HF generate()": hf["throughput_tok_per_sec"], "vLLM": vllm["throughput_tok_per_sec"]},
    {"Metric": "P50 Latency (ms)", "HF generate()": hf["p50_latency_ms"], "vLLM": vllm["p50_latency_ms"]},
    {"Metric": "P95 Latency (ms)", "HF generate()": hf["p95_latency_ms"], "vLLM": vllm["p95_latency_ms"]},
    {"Metric": "GPU Util (%)", "HF generate()": hf["gpu_util_pct"], "vLLM": vllm["gpu_util_pct"]},
])
infer_melt = infer_df.melt(id_vars="Metric", var_name="Backend", value_name="Value")
fig_infer = px.bar(
    infer_melt, x="Metric", y="Value", color="Backend", barmode="group",
