...
with c1:
    st.markdown(METRIC_HTML.format(value="58%", delta="↓ GPU Memory", label="QLoRA vs Full-precision LoRA"), unsafe_allow_html=True)
with c2:
    st.markdown(METRIC_HTML.format(value="70%", delta="↓ Setup Time", label="Config-driven pipeline"), unsafe_allow_html=True)
with c3:
    st.markdown(METRIC_HTML.format(value="20+", delta="↑ Experiments", label="W&B sweep tracked"), unsafe_allow_html=True)
with c4:
    st.markdown(METRIC_HTML.format(value="54→71%", delta="↑ 17pp accuracy", label="MMLU domain accuracy"), unsafe_allow_html=True)
with c5:
    st.markdown(METRIC_HTML.format(value="2.4×", delta="↑ Throughput", label="vLLM vs HF generate()"), unsafe_allow_html=True)
...
with n1:
    st.markdown(NAV_CARD.format(icon="🏋️", title="Training Dashboard",
        desc="Loss curves, LR schedules, GPU metrics from all runs"), unsafe_allow_html=True)
with n2:
    st.markdown(NAV_CARD.format(icon="🤖", title="Model Playground",
        desc="Chat with fine-tuned models live; compare base vs. adapter"), unsafe_allow_html=True)
with n3:
    st.markdown(NAV_CARD.format(icon="📊", title="Evaluation Results",
        desc="MMLU, TruthfulQA, LLM-judge scores and checkpoint progress"), unsafe_allow_html=True)
with n4:
    st.markdown(NAV_CARD.format(icon="🔬", title="Experiment Tracker",
        desc="Sort and filter 20+ W&B experiments; parallel coordinates plot"), unsafe_allow_html=True)
...
if exps:
    ...
    with s3:
        avg_gpu_lora = sum(e["gpu_mem_gb"] for e in exps if e["method"] == "LoRA") / max(1, sum(1 for e in exps if e["method"] == "LoRA"))
        avg_gpu_qlora = sum(e["gpu_mem_gb"] for e in exps if e["method"] == "QLoRA") / max(1, sum(1 for e in exps if e["method"] == "QLoRA"))
    ...
    with s4:
        if eval_data:
            ...
            st.metric("vLLM Speedup", f"{vllm['throughput_tok_per_sec'] / hf['throughput_tok_per_sec']:.1f}×", "vs HF generate()")
