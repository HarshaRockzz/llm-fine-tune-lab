...
def load_checkpoint_results(results_dir: Path) -> pd.DataFrame:
    ...
    for p in sorted(results_dir.glob("*_results.json")):
        with open(p) as f:
            r = json.load(f)
        row = {
            "checkpoint": r.get("checkpoint", p.stem),
        ...
        row["mmlu_social_sciences"] = cats.get("Social Sciences", {}).get(
            "accuracy", None
        )
...
def build_regression_report(
    ...
    if baseline_checkpoint:
        ...
        if baseline_row.empty:
            logger.warning(
                f"Baseline checkpoint {baseline_checkpoint!r} not found — using first row"
            )
            baseline = df.iloc[0]
        else:
    ...
    if output_path:
        ...
        with open(output_path, "w") as f:
            json.dump(
                {k: v for k, v in report.items() if k != "dataframe"}, f, indent=2
            )
    ...
    if regressions:
        ...
        for r in regressions:
            logger.warning(
                f"  {r['checkpoint']} | {r['metric']} | delta={r['delta']:.4f}"
            )
    else:
